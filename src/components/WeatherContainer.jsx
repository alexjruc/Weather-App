...
const WeatherContainer = ({ weather }) => {
    ...
    return (
        <section className="flex flex-col gap-8 text-center justify-center items-center">
            <div className='w-[300px] h-[200px] flex flex-col justify-center tems-center gap-4 text-center text-[#026EED] rounded-3xl '>
                <img className="absolute " src="/bg-card.png" alt="" />
                <div className="grid grid-cols-2 place-items-center relative">
                    <div>
                        <h1 className="text-[3rem]">{changeUnitTemp(weather.main.temp)}</h1>
                        <WeatherStats icon="./src/assets/wind.svg" value={weather.wind.speed} unit="m/s" />
                        <WeatherStats icon="./src/assets/humidity.svg" value={weather.main.humidity} unit="%" />
                        <WeatherStats icon="./src/assets/pressure.svg" value={weather.main.pressure} unit="hPA" />
                    </div>
                    <picture className="absolute -top-8 -right-4">
            ...
            <button onClick={handleClickTemp} className="bg-[#38A1D8] w-1/2 px-4 py-1 rounded-lg text-white">Cambiar a F°</button>
        </section>
    );
};
